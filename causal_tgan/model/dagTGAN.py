...
from causal_tgan.model.module.generator_dag import causal_generator
...
def load_model(model_path, feature_info, transfomer):
    ...
    model = DagTGAN(controller_config, feature_info, transfomer, ori_dag=None, n_gen=0)
    if model.causal_controller is not None:
        checkpoint = torch.load(check_point_folder + '/dagTGAN.pyt', map_location='cpu')
        model.load_checkpoint(checkpoint)

    return model

class DagTGAN(object):
    def __init__(self, config, feature_info, transformer, ori_dag, n_gen, rl_lambda=0.1, pc_alpha=0.05, cuda=True):
    ...
    def _fit_causalGAN(self, train_data, train_options, verbose):
        ...
        self._save_checkpoint(checkpoint, os.path.join(train_options.runs_folder, 'checkpoints'), 'dagTGAN.pyt')
        print_progress(losses_accu)
        print('-' * 40)
    ...
    def train_one_epoch(self, train_data):
        ...
        for steps, data in enumerate(train_data):
            ...
            fake_for_d = self.causal_controller.sample(batch_size).contiguous()  # no logp
            D_fake = self.discriminator(fake_for_d).mean()
            # train with gradient penalty
            gradient_penalty = self.calc_gradient_penalty(real_data, fake_for_d, self.config.pac_num)
            ...
            # ------- G-step (luôn có reward khi update G) -------
            if (steps + 1) % self.config.D_iter == 0:
                # 1) Sinh có log_prob
                fake_for_g, sum_logp = self.causal_controller.sample_with_logprob(self.n_gen)
                fake_data = self.causal_controller.sample(batch_size).contiguous()

                # 2) Reward: chạy PC/SHD trên batch hiện tại
                with torch.no_grad():
                    X_collapsed = self.transformer.inverse_transform(fake_for_g.cpu().numpy())
                    # t_start = time.time()
                    if isinstance(X_collapsed, pd.DataFrame):
                        X_collapsed = X_collapsed.to_numpy()
                    generated_cpdag = pc(data=X_collapsed, alpha=self.pc_alpha, show_progress=False)
                    # t_end = time.time()
                    # t_run = (t_end - t_start) / 60
                    # t_run = round(t_run, 2)
                    # print("run PC for syn data: {}".format(t_run))
                    # 3) Tính reward (SHD) giữa 2 Graph lõi
                    shd_score = SHD(self.dag, generated_cpdag.G).get_shd()
                    reward = -float(shd_score)

                # 3) Adversarial + REINFORCE
                G_adv = -self.discriminator(fake_data).mean()
                G_rl  = -(reward) * sum_logp.mean()

                G_total = G_adv + self.rl_lambda * G_rl
                ...
                # log
                G_losses.append(G_total.data.cpu().numpy())

        losses = {
            'G_cost         ': float(np.mean(G_losses)) if len(G_losses) else 0.0,
            'D_cost         ': float(np.mean(D_losses)) if len(D_losses) else 0.0
        }
        return losses
